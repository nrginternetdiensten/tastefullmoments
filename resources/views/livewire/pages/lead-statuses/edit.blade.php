...
new class extends Component {
    ...
    {
        $this->name = $this->leadStatus->name;
        $this->color_scheme_id = $this->leadStatus->color_scheme_id;
        $this->active = $this->leadStatus->active;
        $this->list_order = $this->leadStatus->list_order;
    }
    ...
    public function save(): void
    {
        ...
        $this->leadStatus->update($validated);

        $this->redirect(route('lead-statuses.index'), navigate: true);
    }
...
<div class="mx-auto max-w-2xl space-y-6">
    <div>
        <flux:heading size="xl">{{ __('Edit Lead Status') }}</flux:heading>
        <flux:subheading>{{ __('Update lead status details') }}</flux:subheading>
    </div>
        ...
        <form wire:submit="save" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="flex items-center justify-between gap-4">
                <flux:button variant="ghost" :href="route('lead-statuses.index')" wire:navigate>
                    {{ __('Cancel') }}
                </flux:button>
