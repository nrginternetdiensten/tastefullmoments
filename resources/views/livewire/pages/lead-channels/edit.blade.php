...
new class extends Component {
    public LeadChannel $leadChannel;
    public string $name = '';
    ...
    {
        $this->name = $this->leadChannel->name;
        $this->color_scheme_id = $this->leadChannel->color_scheme_id;
        $this->active = $this->leadChannel->active;
        $this->list_order = $this->leadChannel->list_order;
    }
    ...
    public function save(): void
    {
        ...
        $this->leadChannel->update($validated);

        $this->redirect(route('lead-channels.index'), navigate: true);
    }
...
<div class="mx-auto max-w-2xl space-y-6">
    <div>
        <flux:heading size="xl">{{ __('Edit Lead Channel') }}</flux:heading>
        <flux:subheading>{{ __('Update lead channel details') }}</flux:subheading>
    </div>
        ...
        <form wire:submit="save" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="flex items-center justify-between gap-4">
                <flux:button variant="ghost" :href="route('lead-channels.index')" wire:navigate>
                    {{ __('Cancel') }}
                </flux:button>
