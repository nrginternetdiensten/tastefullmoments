...
new class extends Component {
    ...
    {
        $this->name = $this->leadCategory->name;
        $this->color_scheme_id = $this->leadCategory->color_scheme_id;
        $this->active = $this->leadCategory->active;
        $this->list_order = $this->leadCategory->list_order;
    }
    ...
    public function save(): void
    {
        ...
        $this->leadCategory->update($validated);

        $this->redirect(route('lead-categories.index'), navigate: true);
    }
...
<div class="mx-auto max-w-2xl space-y-6">
    <div>
        <flux:heading size="xl">{{ __('Edit Lead Category') }}</flux:heading>
        <flux:subheading>{{ __('Update lead category details') }}</flux:subheading>
    </div>
        ...
        <form wire:submit="save" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:input
                ...
                required
                autofocus
                :placeholder="__('Web Development')"
            />
            ...
            <div class="flex items-center justify-between gap-4">
                <flux:button variant="ghost" :href="route('lead-categories.index')" wire:navigate>
                    {{ __('Cancel') }}
                </flux:button>
