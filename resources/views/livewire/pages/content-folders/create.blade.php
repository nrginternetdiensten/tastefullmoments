...
new class extends Component {
    ...
    public function save(): void
    {
        ...
        ContentFolder::create($validated);
...
<div class="mx-auto max-w-2xl space-y-6">
    <div>
        <flux:heading size="xl">{{ __('Create Content Folder') }}</flux:heading>
        <flux:subheading>{{ __('Add a new content folder') }}</flux:subheading>
    </div>
    ...
    <form wire:submit="save" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div class="flex justify-between">
            <flux:button :href="route('content-folders.index')" wire:navigate variant="ghost">
                {{ __('Cancel') }}
            </flux:button>
