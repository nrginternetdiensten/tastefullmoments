...
new class extends Component {
    ...
    {
        $this->name = $this->contentFolder->name;
        $this->color_scheme_id = $this->contentFolder->color_scheme_id;
        $this->active = $this->contentFolder->active;
        $this->list_order = $this->contentFolder->list_order;
    }
    ...
    public function save(): void
    {
        ...
        $this->contentFolder->update($validated);

        $this->redirect(route('content-folders.index'), navigate: true);
    }
...
<div class="mx-auto max-w-2xl space-y-6">
    <div>
        <flux:heading size="xl">{{ __('Edit Content Folder') }}</flux:heading>
        <flux:subheading>{{ __('Update content folder details') }}</flux:subheading>
    </div>
    ...
    <form wire:submit="save" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div class="flex items-center justify-between gap-4">
            <flux:button variant="ghost" :href="route('content-folders.index')" wire:navigate>
                {{ __('Cancel') }}
            </flux:button>
