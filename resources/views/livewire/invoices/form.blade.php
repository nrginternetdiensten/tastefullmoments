...
new class extends Component {
    ...
    public function mount(?Invoice $invoice = null): void
    {
        if ($invoice && $invoice->exists) {
            $this->invoice = $invoice->load('lines.tax');
    ...
    public function save(): void
    {
        ...
        foreach ($validated['lines'] as $lineData) {
            ...
            if (!empty($lineData['id'])) {
                ...
                $this->invoice->lines()->create($lineSaveData);
            }
        }
...
<div class="space-y-6">
    ...
    <form wire:submit="save" class="space-y-6">
        <flux:field>
            <flux:label>{{ __('Factuurnummer') }}</flux:label>
            <flux:input wire:model="invoice_id" placeholder="{{ __('INV-12345') }}" />
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                <flux:heading size="lg">{{ __('Factuurregels') }}</flux:heading>
        ...
        <div class="flex gap-3">
            <flux:button type="submit" variant="primary">
                {{ $invoice ? __('Opslaan') : __('Aanmaken') }}
            </flux:button>
            <flux:button type="button" :href="route('invoices.index')" wire:navigate variant="ghost">
                {{ __('Annuleren') }}
            </flux:button>
        </div>
    </form>
