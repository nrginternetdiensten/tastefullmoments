<x-layouts.frontend :title="$content->seo_title ?? $content->name">
    <div class="min-h-screen py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($content->folder)
                <nav class="mb-8 flex" aria-label="Breadcrumb">
                    <ol class="flex items-center gap-2 text-sm">
                        ...
                        <li>
                            <span class="text-zinc-700 dark:text-zinc-300">{{ $content->folder->name }}</span>
                        </li>
                        <li class="text-zinc-400 dark:text-zinc-600">/</li>
                        <li>
                            <span class="text-zinc-900 dark:text-zinc-100 font-medium">{{ $content->name }}</span>
                        </li>
                    </ol>
                </nav>
            @endif
            ...
            <div class="mb-8">
                @if($content->type)
                    <div class="mb-3">
                        <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-medium {{ $content->type->colorScheme->bg_class ?? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200' }}">
                            {{ $content->type->name }}
                        </span>
                    </div>
                @endif
